...
class MovieDetailViewController: UIViewController {
    ...
    @IBAction func toggleFavorite(sender: AnyObject) {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            guard let code = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int,
                let message = parsedResult[Constants.TMDBResponseKeys.StatusMessage] as? String else {
                displayError("Cannot find key '\(Constants.TMDBResponseKeys.StatusCode)' in \(parsedResult)")
                return
            }
            
            /* 6. Use the data! */
            if ((shouldFavorite && (code == 1 || code == 12)) || (!shouldFavorite && code == 13)) {
